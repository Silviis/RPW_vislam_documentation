...
\def\myfititle{Visual Intertial SLAM on custom hardware sensor box}
\def\myentitle{Visual Intertial SLAM on custom hardware sensor box}
\def\myauthor{Juuso Järvenpää\\Eetu Silvennoinen\\Joona Leivo}
\def\myfisubtitle{}
\def\myensubtitle{}
\def\myfithesistype{ROBO.666 Project Report}
\def\myenthesistype{ROBO.666 Project Report}
\def\myexaminers{}
...
\def\myenprogrammename{Automation technology}
\def\myfikeywords{}
\def\myenkeywords{}
...
\preface{tex/preface}{Tampereella}
...
\newcommand{\addpicture}[2]{
    \begin{figure}[H]
    \includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{figures/#1}
    \caption{#2}
    \label{fig:#1}
    \end{figure}
} % Macro command to easily add pictures that are fitted into text width.

\newcommand{\addsvg}[2]{
    \begin{figure}[H]
    \includesvg[width=\textwidth,height=\textheight,keepaspectratio]{figures/#1}
    \caption{#2}
    \label{fig:#1}
    \end{figure}
} % Macro command to easily add svgs that are fitted into text width.
...
\input{tex/system-overview}
\input{tex/sensor-setup.tex}
\input{tex/slam}
\input{tex/cicd.tex}
