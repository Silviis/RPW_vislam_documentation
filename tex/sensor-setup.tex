...
independent image sensors mounted with a fixed baseline (see Figure \ref{fig:imx219-83.jpg}.
The cameras are interfaced with the Jetson Xavier NX through the CSI interface,
enabling low-latency and high-bandwidth image acquisition suitable for real-time visual-inertial SLAM.
...
and $k_4$. Focal lengths and principal points are in pixels. We chose two different tools for camera calibration; Kalibr and camera\_calibration toolboxes, which both have ROS implementations. In addition to aforementioned parameters, camera\_calibration toolbox also computed rectification matrices for both cameras. This is crucial for VI-SLAM algorithm to be able to detect same features in both images, as they project two images from separate cameras onto a common image plane so that corresponding points on both images have the same row coordinate.  Kalibr toolbox was used to calculate the extrinsic parameters of the stereo setup, i.e. rotation matrix $\textbf{R}$ and translation vector $\textbf{t}$ from one camera to another, which was needed for extrinsic calibration of the whole setup. Note, that the results of the extrinsic calibration are given in consensus with the ROS frame convention for image plane; $z$-axis forward, $x$-axis right and $y$-axis down.

Both calibrations were done with the same about 1 minute long dataset, where a $8\times6$ checkerboard pattern with a square size of $40.5 \,\mathrm{mm}$ was moved in front of the cameras, so that it was visible in both images at the same time. The results of calibration can be seen in 

\subsection{IMU Intrinsic Calibration}\label{sec:imu_intrinsic_calibration}
Consumer-grade IMUs, such as ICM20948, tend to be noisy. The effect of the noise on VI-SLAM performance is reduced by estimating the characteristics of the different noise components corrupting the IMU measurements. The noise parameters that are estimated for our chosen VI-SLAM method are additive white noise densities $\sigma_a$ and $\sigma_g$, and the slowly varying sensor biases $\sigma_{b_a}$ and $\sigma_{b_a}$, also called "random walk". Lower indices $a$ and $g$ denote accelerometer and gyroscope respectively.

These parameters were estimated using Allan variance method. For calibration, 11 hours of stationary IMU data were recorded. In short, Allan variance $\sigma_y^2$ and Allan deviation $\sigma_y$ are functions of sample-time $\tau$, which can then plotted on a log-log plot to determine the noise characteristics, as seen in figures \ref{fig:gyro.png} and \ref{fig:acceleration.png}. Noise density is estimated by fitting a line with slope $-\frac{1}{2}$ on the log-log plot (magenta lines in figures \ref{fig:gyro.png} and \ref{fig:acceleration.png}, one for each axis of the sensor).  Then, noise density is read from the point where the fitted line intersects $\tau=1\,\mathrm{s}$. Random walk is estimated by fitting a line with slope $\frac{1}{2}$ on the log-log plot (yellow lines in figures \ref{fig:gyro.png} and \ref{fig:acceleration.png}, one for each axis of the sensor). Then, random walk is read from the point where the fitted line intersects $\tau=3\,\mathrm{s}$. 

We chose allan\_variance\_ros toolbox as our calibration tool. On our dataset, there are no regions where the slope would reach $\frac{1}{2}$, but allan\_variance\_ros handles this via interpolation. The results of the calibration can be seen in 

\addpicture{gyro.png}{Allan deviation plot of gyroscope measurements.}
\addpicture{acceleration.png}{Allan deviation plot of accelerometer measurements.}

\subsection{Extrinsic camera-IMU calibration}\label{sec:extrinsic_calibration}
Last, we determined the extrinsic parameters of the whole setup, i.e. where the cameras are in relation to the IMU.
The extrinsic calibration was also performed using Kalibr toolbox \cite{extending_kalibr} \cite{furgale_unified_2013}. The calibration process used the intrinsic parameters from prior calibration steps, as well as the transform from camera 0 to camera 1. The calibration was done by moving and rotating the sensor setup in front of the stationary calibration target (same checkerboard). We attempted to do the calibration with different lengthed datasets, but did not manage to get a good result. Calibration results were logical when comparing to the physical setup, except for translation along the $x$ axis. For every calibration 
