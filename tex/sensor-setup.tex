...
These parameters were estimated using Allan variance method. For calibration, 11 hours of stationary IMU data were recorded. We chose allan\_variance\_ros toolbox for calibration. In short, Allan variance $\sigma_y^2$ and Allan deviation $\sigma_y$ are functions of sample-time $\tau$, which can then plotted on a log-log plot to determine the noise characteristics, as seen in \ref{fig:allan_gyro} and \ref{fig:allan_accel}. Noise densities $\sigma_a$ can be estima

\addpicture{gyro.png}{Allan deviation plot of gyroscope measurements.}\label{fig:allan_gyro}
\addpicture{acceleration.png}{Allan deviation plot of accelerometer measurements.}\label{fig:allan_accel}
...
the Kalibr toolbox \cite{extending_kalibr} \cite{furgale_unified_2013}. This process involved capturing synchronized data
